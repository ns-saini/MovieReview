...
df = pandas.read_csv('data_titletoname.tsv', usecols=['titleId', 'title', 'language'],
                     header=0, sep='\t')
...
df.rename(columns={"titleId": "id", "title": "title", "language": "language"},
          inplace=True)

for index, row in df.iterrows():
    op = row.to_json()
    channel.basic_publish(exchange='titletoname_exchange', routing_key='crud_service_titletoname_queue', body=op)
    print(" [x] Sent %r" % op)
