...
df = pandas.read_csv('data_ratings.tsv', usecols=['tconst', 'averageRating', 'numVotes'],
                     header=0, sep='\t')
...
df.rename(columns={"tconst": "id", "averageRating": "average_rating", "numVotes": "num_of_votes"},
          inplace=True)

for index, row in df.iterrows():
    op = row.to_json()
    channel.basic_publish(exchange='ratings_exchange', routing_key='crud_service_ratings_queue', body=op)
    print(" [x] Sent %r" % op)
